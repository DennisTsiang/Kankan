...
function handleTicketDragStart(socket) {
  return function handleTicketDragStart(e) {
...
function handleTicketDragLeave(socket) {
  return function handleTicketDragLeave(e) {
    e.toElement.style.border = "";
    $(e.toElement).closest('.ticket-column')[0].style.border = ""
  }
}

function handleTicketDragOver(socket) {
  return function handleTicketDragOver(e) {
    e.preventDefault();
    $(e.toElement).closest('.ticket-column')[0].style.border = "thick solid #0000FF"
  }
}

function handleTicketDrop(socket) {
  return function handleTicketDrop(e) {
    e.preventDefault();
...
let start_column_id;
function handleColumnDragStart(socket) {
  return function handleColumnDragStart(event) {
...
function handleColumnDragOver(socket) {
  return function handleColumnDragOver(event) {
...
function handleColumnDrop(socket) {
return function handleColumnDrop(event) {
    event.preventDefault();
